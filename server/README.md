...
If the new frontend variable is empty, Edge cloud requests continue using `VITE_API_BASE_URL`. Keep that existing variable pointed at the general compute backend for PDF, Piper, Kokoro and other APIs. This service implements only `/api/etts` and its preflight.
