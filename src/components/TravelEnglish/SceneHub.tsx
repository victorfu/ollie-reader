...
export function SceneHub({ scene, onBack, onSelectActivity, travelProgress }: SceneHubProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Scene Hero Card */}
      <div
        className={`rounded-2xl p-5 sm:p-6 border border-border-hairline shadow-elevated backdrop-blur-xl ${scene.colorClass}`}
      >
        <span className="text-5xl">{scene.emoji}</span>
        <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight mt-3">{scene.title}</h2>
        <p className="text-muted-foreground text-sm">{scene.titleChinese}</p>
        ...
        {funFact && (
          <motion.div
            ...
            className="mt-4 p-3 bg-white/50 dark:bg-white/5 rounded-xl border border-border-hairline"
          >
            <div className="flex items-start gap-2">
              <span className="text-lg">{funFact.emoji}</span>
              <div>
                <p className="text-xs font-medium text-warning mb-0.5">
                  💡 Did you know?
                </p>
                <p className="text-sm">{funFact.english}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{funFact.chinese}</p>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div>
        <h3 className="text-lg font-semibold mb-3">
          🎯 你的任務 <span className="text-sm font-normal text-muted-foreground">Your Missions</span>
        </h3>
          ...
          {activities.map((act) => {
            ...
            return (
              <motion.button
                ...
                className={`surface-card relative flex flex-col items-center gap-2 p-4 rounded-2xl text-center min-h-[120px] justify-center transition-all ${
                  isLocked
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{act.labelEn}</p>
                </div>
                {/* Status badge */}
                <div className="mt-1">
                  {status === "locked" && (
