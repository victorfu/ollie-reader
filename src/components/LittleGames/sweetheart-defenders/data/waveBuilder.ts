 ...
 * 波表產生器。
 *
 * 五張地圖 × 15 波 = 75 波，全部手寫大概兩百多筆設定，之後想調難度就得逐筆
 * 改——所以改成用「節奏」描述：每張地圖只說自己有哪些怪、整體強度多少，
 * 波次的形狀由這裡統一產生。要重新平衡就是改一個 intensity 數字。
 *
 * 節奏以五波為一循環，Boss 固定落在第 5、10、15 波：
...
const DEFAULT_WAVE_COUNT = 15;
/** 每過一波，敵人數量成長多少。血量成長另外由 waves.ts 負責。 */
const COUNT_GROWTH_PER_WAVE = 0.12;
...
function groupsForWave(plan: WavePlan, index: number): WaveGroup[] {
  ...
  switch (index % 5) {
    ...
    case 0:
      return spread(plan, [
        { kind: pick(plan.swarm, index), count: amount(7), gapMs: 700, delayMs: 0 },
        {
    ...
    case 1:
      return spread(plan, [
        { kind: pick(plan.swarm, index), count: amount(6), gapMs: 650, delayMs: 0 },
        { kind: pick(plan.rush, index), count: amount(4), gapMs: 420, delayMs: 2500 },
        { kind: pick(plan.tank, index), count: amount(1), gapMs: 2200, delayMs: 6000 },
      ]);

    // 重甲：慢慢走的肉盾，配一點雜兵騷擾
    case 2:
      return spread(plan, [
        { kind: pick(plan.tank, index), count: amount(3), gapMs: 2000, delayMs: 0 },
        { kind: pick(plan.swarm, index), count: amount(5), gapMs: 700, delayMs: 1500 },
      ]);

    // 快攻：間隔很短的一波衝刺
    case 3:
      return spread(plan, [
        { kind: pick(plan.rush, index), count: amount(6), gapMs: 340, delayMs: 0 },
        { kind: pick(plan.rush, index + 1), count: amount(3), gapMs: 400, delayMs: 3500 },
      ]);
    ...
    default: {
      ...
      return spread(plan, [
        ...
        { kind: pick(plan.tank, index), count: amount(2), gapMs: 2200, delayMs: 3000 },
        { kind: pick(plan.swarm, index), count: amount(5), gapMs: 650, delayMs: 5500 },
      ]);
    }
  }
}
