...
const API_URL = 'https://purism-ev-bot-1027147244019.asia-east1.run.app/api/pdf/extract'
  ...
  const header = useMemo(() => {
    return (
      <div className="mb-6">
        ...
        <p className="mt-2 text-gray-300">上傳 PDF，呼叫 API 解析英文文字，點擊單字即可發音。</p>
      </div>
    )
  }, [])
      ...
      {result && (
        <div className="mt-8 space-y-6">
          <div className="text-gray-300">
            <div className="font-semibold">檔名：<span className="text-white">{result.filename}</span></div>
            <div>總頁數：{result.total_pages}</div>
          </div>

          <div className="space-y-8">
            {result.pages.map((p) => (
              <div key={p.page_number} className="bg-gray-800/50 border border-gray-700 rounded-xl p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-xl font-semibold text-white">Page {p.page_number}</h3>
