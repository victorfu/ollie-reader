...
**決策:本功能不加自動化測試。** web app 目前只有用 Node 內建 `node:test` 跑的 leaf util 測試(顯式 `.ts` 副檔名);`localBackend.ts` 屬於 Vite module graph(extensionless import + `import.meta.env`),Node 原生 runner 無法解析,而專案刻意不引入 vitest 等框架。因此改以下列方式驗證:

- **型別檢查**:`npm run build`(`tsc -b`)必須通過。
- **Lint**:`npm run lint` 必須通過。
- **手動驗證**(`npm run dev`):
  - 三模式在 Settings 可切換,重整後保留(localStorage)。
  - `cloud` 模式:DevTools Network 不應出現對 `127.0.0.1:8765` 的請求。
  - `auto` 模式:啟動桌面 sidecar 後按「重新偵測」→ 狀態顯示「本機」;PDF/TTS 走本機。關掉 sidecar 後做一次 PDF/TTS → 自動接回雲端(retry 一次)。
  - `local` 模式且 sidecar 未開:PDF 顯示「本機未連線」訊息,不退雲端。
